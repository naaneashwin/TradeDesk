...
export default function App() {
  ...
  const handleDeleteChecklistItem = withSync(async (id) => {
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = "tradedeck-backup.json";
    a.click();
  ...
  const importData = (e) => {
    ...
    r.onload = (ev) => {
      ...
      if (missing.length) {
        setImportError(
          `Incompatible file. Missing columns: ${missing.join(", ")}`,
        );
        e.target.value = "";
        return;
      }
      ...
      const col = (cells, k) =>
        cells[headers.indexOf(k)]?.replace(/^"|"$/g, "").trim() ?? "";
      ...
      rows.slice(1).forEach((row, i) => {
        ...
        if (!strategyId) {
          errors.push(`Row ${rowNum}: Strategy "${get("Strategy")}" not found`);
          return;
        }
        if (!get("Variant")) {
          errors.push(`Row ${rowNum}: Variant is empty`);
          return;
        }
        if (!get("Date")) {
        ...
        if (!get("Instrument")) {
          errors.push(`Row ${rowNum}: Instrument is empty`);
          return;
        }
        ...
        if (!["win", "loss", "breakeven"].includes(outcome)) {
          errors.push(`Row ${rowNum}: Outcome must be win, loss, or breakeven`);
          return;
        }
        if (!parseNum(get("Entry"))) {
          errors.push(`Row ${rowNum}: Entry price is invalid`);
          return;
        }
        if (!parseNum(get("Exit"))) {
          errors.push(`Row ${rowNum}: Exit price is invalid`);
          return;
        }
        if (!parseNum(get("Qty"))) {
          errors.push(`Row ${rowNum}: Qty is invalid`);
          return;
        }
        ...
        tradesToInsert.push({
          ...
          variant: get("Variant"),
          checklistScore: {
            done: parseInt(scoreParts[0]) || 0,
            total: parseInt(scoreParts[1]) || 0,
          },
            ...
            <Route
              ...
              element={
                <Library
                  strats={strats}
                  trades={trades}
                  onOpen={(st) => navigate(`/tradedesk/strategies/${st.id}`)}
                  onUpsert={handleUpsertStrategy}
                  onDelete={handleDeleteStrategy}
                  checklistItems={checklistItems}
                  onUpsertChecklistItem={handleUpsertChecklistItem}
                />
              }
            />
            <Route
              path="/tradedesk/strategies/:id"
              element={
                <ChecklistRoute
                  strats={strats}
                  trades={trades}
