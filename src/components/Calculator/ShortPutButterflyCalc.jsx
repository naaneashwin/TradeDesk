...
// Short Put Butterfly (mirror of long put — collect credit):
//   Sell 1 upper put, Buy 2 middle puts, Sell 1 lower put
//   Net Credit = (upper premium + lower premium) - 2×middle premium
//   Lower BE = lower strike + net credit
//   Upper BE = upper strike - net credit
//   Max Profit = net credit × qty (price ≤ lower or ≥ upper at expiry)
//   Max Loss   = (wingWidth − net credit) × qty (price pins at middle)

export default function ShortPutButterflyCalc() {
  ...
  const netCreditRaw = premiumsOk ? uP + lP - 2 * mP : null;
  ...
  const pnlFn = (spot) =>
    -(Math.max(0, uS - spot) - 2 * Math.max(0, mS - spot) + Math.max(0, lS - spot)) + netCredit;
  ...
  const dissectSteps = valid ? [
    ...
    {
      label: "Net Credit",
      formula: `upperPremium + lowerPremium − 2×middlePremium = ${fmtINR(uP)} + ${fmtINR(lP)} − 2×${fmtINR(mP)}`,
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <div style={CARD}>
        <p style={SEC_TITLE}>Short Put Butterfly Inputs</p>
        <p style={{ fontSize: 12, color: "var(--text-3)", marginTop: -12, marginBottom: 16 }}>
          Sell 1 upper put · Buy 2 middle puts · Sell 1 lower put (collect credit)
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <Field label="Lower Strike — Sell (₹)"    k="lowerStrike"   placeholder="490" hint="Lower wing put (sold)"            form={form} onChange={onChange} />
          <Field label="Lower Premium (₹)"          k="lowerPremium"  placeholder="8"   hint="Premium received"                 form={form} onChange={onChange} />
          <Field label="Middle Strike — Buy ×2 (₹)" k="middleStrike"  placeholder="500" hint="Body puts bought twice (ATM)"     form={form} onChange={onChange} />
          <Field label="Middle Premium (₹)"         k="middlePremium" placeholder="14"  hint="Premium paid per contract"        form={form} onChange={onChange} />
          <Field label="Upper Strike — Sell (₹)"    k="upperStrike"   placeholder="510" hint="Upper wing put (sold)"            form={form} onChange={onChange} />
          <Field label="Upper Premium (₹)"          k="upperPremium"  placeholder="22"  hint="Premium received"                 form={form} onChange={onChange} />
      ...
      {valid && (
        <>
          <div style={{ background: "var(--green-light)", border: "1px solid rgba(45,122,95,0.2)", borderRadius: 12, padding: 24, marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: "rgba(45,122,95,0.7)", margin: 0 }}>Safe zone: price stays outside {fmtINR(lowerBE)} – {fmtINR(upperBE)}</p>
          </div>
          ...
          <TradeExpectation
            ...
            exitRule="Exit if price is stalling near the middle strike — the max loss occurs right at a pin. Don't wait."
          />

          <DissectPanel
            steps={dissectSteps}
            legs={[
              { label: "Upper Put (Sell)",  action: "Sell", qty: 1, type: "Put", strike: uS, premium: uP, desc: "Collects premium on the upper wing. Part of the net credit received. Profits when price stays above the upper put strike or breaks far below the lower wing." },
              { label: "Middle Put (Buy)",  action: "Buy",  qty: 2, type: "Put", strike: mS, premium: mP, desc: "The core volatility legs — buying two ATM puts gives you direct downside exposure. They pay off significantly if price makes a large move away from the middle strike." },
              { label: "Lower Put (Sell)",  action: "Sell", qty: 1, type: "Put", strike: lS, premium: lP, desc: "Collects premium on the lower wing. Together with the upper short put, the combined credit from both wings funds the two long middle puts. Caps maximum loss on an extreme downside move." },
            ]}
            lotQty={qty}
          />
          ...
          <PayoffChart
            ...
            title="Short Put Butterfly — Payoff at Expiry"
          />
          <PLSimulator
            pnlFn={pnlFn}
            qty={qty}
            ...
            isShort
            legFns={[
              (spot) => uP - Math.max(0, uS - spot),
              (spot) => Math.max(0, mS - spot) - mP,
              (spot) => Math.max(0, mS - spot) - mP,
              (spot) => lP - Math.max(0, lS - spot),
            ]}
            legLabels={["Short upper put", "Long middle put (1)", "Long middle put (2)", "Short lower put"]}
          />
        </>
      )}
    </div>
