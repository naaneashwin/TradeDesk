...
export default function LogModal({
  ...
  const [form, setForm] = useState({
    ...
    entryPrice: isEdit
      ? String(editTrade.entryPrice)
      : (prefill?.entryPrice ?? ""),
    stopLoss: isEdit
      ? editTrade.initialSl != null
        ? String(editTrade.initialSl)
        : ""
      : "",
    qty: isEdit ? String(editTrade.qty) : (prefill?.qty ?? ""),
    tradeType: isEdit
      ? (editTrade.tradeType ?? "eq_delivery")
      : (prefill?.tradeType ?? "eq_delivery"),
    exchange: isEdit
      ? (editTrade.exchange ?? "NSE")
      : (prefill?.exchange ?? "NSE"),
    commission: isEdit
      ? editTrade.commission != null
        ? String(editTrade.commission)
        : ""
      : "",
    commissionAuto: true, // auto-fill from brokerage calc unless user edits manually
    screenshotUrl: isEdit ? (editTrade.screenshotUrl ?? "") : "",
    planThesis: isEdit
      ? (editTrade.planThesis ?? "")
      : (prefill?.planThesis ?? ""),
    planTarget: isEdit
      ? editTrade.planTarget != null
        ? String(editTrade.planTarget)
        : ""
      : prefill?.planTarget != null
        ? String(prefill.planTarget)
        : "",
    planStop: isEdit
      ? editTrade.planStop != null
        ? String(editTrade.planStop)
        : ""
      : prefill?.planStop != null
        ? String(prefill.planStop)
        : "",
  ...
  const fExit = (id, k, v) =>
    setExits((prev) => prev.map((e) => (e.id === id ? { ...e, [k]: v } : e)));
  const addExit = () =>
    setExits((prev) => [
      ...prev,
  ...
  const initialRisk =
    entryPrice > 0 && stopLoss > 0 ? Math.abs(entryPrice - stopLoss) : 0;
  ...
  const exitsCalc = exits.map((e) => {
    ...
    const pnl =
      ep && qty && entryPrice
        ? (ep - entryPrice) * qty * (dir === "short" ? -1 : 1)
        : null;
    return { ...e, pnlCalc: pnl };
  });

  const totalExitQty = exitsCalc.reduce(
  ...
  const fullyExited = entryQty > 0 && totalExitQty >= entryQty;
  const weightedAvgExit =
    totalExitQty > 0
      ? exitsCalc.reduce(
          (s, e) =>
            s + (parseFloat(e.exitPrice) || 0) * (parseFloat(e.qty) || 0),
          0,
        ) / totalExitQty
      : 0;
  const outcome =
    hasExits && fullyExited
      ? totalPnl > 0
        ? "win"
        : totalPnl < 0
          ? "loss"
  ...
  const save = () => {
    ...
    const newCustom = payload.exits
      ...
      .filter(
        (s) =>
          s &&
          !PRESET_EXIT_STRATEGIES.includes(s) &&
          !customStrategies.includes(s),
      );
    newCustom.forEach((label) => {
      addCustomExitStrategy(label).catch(() => {});
    });
    if (newCustom.length)
      setCustomStrategies((prev) => [...prev, ...newCustom]);
    ...
    >
        ...
        {strats && strats.length > 0 && !isEdit ? (
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setSelectedStrategy(
                  strats.find((s) => s.id === e.target.value) ?? strategy,
                )
              }
            >
              {strats.map((s) => (
          ...
          >
            ...
            {score ? ` · Checklist ${score.done}/${score.total}` : ""}
          </p>
        )}
      </div>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#d97706" }}>
              Checklist incomplete ({score?.done}/{score?.total} items)
            </div>
            <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 2 }}>
              Less than 50% of checklist items are checked. Click Save again to
              log anyway.
            </div>
          </div>
          <button
      ...
      >
        <Field label="Date">
          <input
            ...
            className="t-inp"
            value={form.date}
            onChange={(e) => fEntry("date", e.target.value)}
          />
        </Field>
        <Field label="Instrument">
          <SymbolSearch
            value={form.instrument}
            onChange={(v) => fEntry("instrument", v)}
          />
        </Field>
        <Field label="Direction">
          <select
            ...
            onChange={(e) => fEntry("direction", e.target.value)}
          >
        ...
        <Field label="Entry Price (₹)">
          <input
            ...
            onChange={(e) => fEntry("entryPrice", e.target.value)}
            placeholder="0.00"
          />
        </Field>
        <Field label="Stop Loss (₹)">
          <input
            ...
            className="t-inp font-mono"
            value={form.stopLoss}
            onChange={(e) => fEntry("stopLoss", e.target.value)}
            placeholder="0.00"
          />
        </Field>
        <Field label="Total Quantity">
          <input
            ...
            className="t-inp font-mono"
            value={form.qty}
            onChange={(e) => fEntry("qty", e.target.value)}
            placeholder="0"
          />
        </Field>
      </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {entryQty > 0 && (
            <span
              style={{
                fontSize: 11,
                color: exitedPct >= 100 ? "var(--green)" : "var(--text-3)",
                fontFamily: "JetBrains Mono, monospace",
              }}
            >
        ...
        >
          <div
            style={{
              height: "100%",
              width: `${exitedPct}%`,
              background: exitedPct >= 100 ? "var(--green)" : "#3b82f6",
              borderRadius: 3,
          ...
          return (
            ...
            >
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns: `1fr 1fr 1fr 80px${exits.length > 1 ? " 28px" : ""}`,
                  gap: 12,
                  alignItems: "end",
                }}
              >
                <Field label={`Exit ${i + 1} — Qty`}>
                  <input
                    ...
                    className="t-inp font-mono"
                    value={e.qty}
                    onChange={(ev) => fExit(e.id, "qty", ev.target.value)}
                    placeholder="0"
                  />
                </Field>
                <Field label="Exit Price (₹)">
                  <input
                    ...
                    className="t-inp font-mono"
                    value={e.exitPrice}
                    onChange={(ev) => fExit(e.id, "exitPrice", ev.target.value)}
                    placeholder="0.00"
                  />
                </Field>
                <Field label="Exit Date">
                  <input
                    ...
                    className="t-inp"
                    value={e.exitDate}
                    onChange={(ev) => fExit(e.id, "exitDate", ev.target.value)}
                  />
                </Field>
                <div>
                  <p
              ...
              <div style={{ marginTop: 10 }}>
                <Field label="Exit Strategy">
                  <input
                    ...
                    onChange={(ev) =>
                      fExit(e.id, "exitStrategy", ev.target.value)
                    }
                    placeholder="Select or type a reason…"
                    autoComplete="off"
                  />
                  <datalist id={`exit-strategy-list-${e.id}`}>
                    {PRESET_EXIT_STRATEGIES.map((s) => (
      ...
      {hasExits && (
        ...
        >
          <div>
            ...
            >
              Total Realized P&L
              {exits.filter((e) => e.exitPrice && e.qty).length > 1
                ? ` · ${exits.filter((e) => e.exitPrice && e.qty).length} exits`
                : ""}
            </p>
            <p
              style={{
            ...
            {rMult != null && (
              <p
                style={{
                  fontSize: 12,
                  fontWeight: 700,
                  color: rMult > 0 ? "var(--green)" : "var(--red)",
                  margin: "4px 0 0",
                  fontFamily: "JetBrains Mono, monospace",
                }}
              >
      ...
      <div
        style={{
          ...
          marginBottom: 16,
          padding: "12px 14px",
          background: form.mock ? "rgba(217,119,6,0.06)" : "var(--surface-2)",
          border: `1px solid ${form.mock ? "rgba(217,119,6,0.25)" : "var(--border)"}`,
          borderRadius: 10,
          cursor: "pointer",
        }}
              ...
              >
                <div
                  style={{
                    ...
                    height: 16,
                    borderRadius: "50%",
                    border: `2px solid ${active ? (opt === "mock" ? "#d97706" : "var(--green)") : "var(--border)"}`,
                    display: "flex",
                    alignItems: "center",
                  ...
                  {active && (
                    <div
                      style={{
                        ...
                        height: 7,
                        borderRadius: "50%",
                        background: opt === "mock" ? "#d97706" : "var(--green)",
                      }}
                    />
                  )}
                </div>
      ...
      <Field label="Notes">
        <textarea
          className="t-inp"
          style={{ height: 72, resize: "vertical", marginBottom: 14 }}
          value={form.notes}
          onChange={(e) => fEntry("notes", e.target.value)}
      ...
      <Field label="Tags (comma-separated)">
        <input
          ...
          value={form.tags}
          onChange={(e) => fEntry("tags", e.target.value)}
          placeholder="e.g. breakout, momentum, FOMO mistake"
        />
      </Field>
      ...
      <Field label="Thesis / Reason to Trade">
        <textarea
          className="t-inp"
          style={{ height: 60, resize: "vertical", marginBottom: 14 }}
          value={form.planThesis}
          onChange={(e) => fEntry("planThesis", e.target.value)}
      ...
      >
        <Field label="Target Price (₹)">
          <input
            ...
            onChange={(e) => fEntry("planTarget", e.target.value)}
            placeholder="0.00"
          />
        </Field>
        <Field label="Planned Stop (₹)">
          <input
            ...
            className="t-inp font-mono"
            value={form.planStop}
            onChange={(e) => fEntry("planStop", e.target.value)}
            placeholder="0.00"
          />
        </Field>
      </div>
      ...
      >
        <Field label="Trade Type">
          <select
            ...
            onChange={(e) => fEntry("tradeType", e.target.value)}
          >
      ...
      {brokerageResult && entryPrice > 0 && entryQty > 0 && (
        ...
        >
          ...
          >
            ...
            >
              ({TRADE_TYPE_LABELS[form.tradeType]} · {form.exchange})
            </span>
          </div>
      ...
      >
        <Field
          label={`Commission / Brokerage (₹)${form.commissionAuto ? " — auto" : " — manual"}`}
        >
          ...
          {!form.commissionAuto && (
            <button
              type="button"
              onClick={() => setForm((p) => ({ ...p, commissionAuto: true }))}
              style={{
                marginTop: 4,
                fontSize: 10,
                color: "#3b82f6",
        ...
        <Field label="Screenshot URL">
          <input
            ...
            onChange={(e) => fEntry("screenshotUrl", e.target.value)}
            placeholder="https://…"
          />
        </Field>
      </div>
